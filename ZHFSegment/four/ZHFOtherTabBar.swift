...
class ZHFOtherTabBar: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        //1. 获取json 文件路径
        guard let jsonPath = Bundle.main.path(forResource: "tabbar1.json", ofType: nil) else {
    ...
    private func addChildViewController(childVCName: String,title : String, imagename : String) {
        if childVCName == "MainFourVC" {
            let mainFourVC:MainFourVC = MainFourVC()
            mainFourVC.title = title
            mainFourVC.tabBarItem.image = UIImage.init(named: "fire")!.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
            mainFourVC.tabBarItem.selectedImage = UIImage.init(named: "fire_highlighted")!.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
            mainFourVC.tabBarItem.imageInsets = UIEdgeInsets.init(top: 0, left: 0, bottom: 0, right: 0)
            mainFourVC.tabBarItem.titlePositionAdjustment = UIOffset(horizontal: 0, vertical: -3)
            let childNav = UINavigationController(rootViewController: mainFourVC)
            addChild(childNav)
        }
